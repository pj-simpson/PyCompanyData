...
class AccountsHandler(BaseHandler):
    def get_all_accounts(self, company_id: str, query: str, order_by: str) -> typing.List[Account]:
        path = f"{self.path}{company_id}/data/accounts"
        return self._get_all_pages(Account, path, query=query, orderBy=order_by)

    def get_pageof_accounts(self, company_id: str, page_number: int, page_size: int, query: str,
                            order_by: str) -> PaginatedResponse[Account]:
        path = f"{self.path}{company_id}/data/accounts"
        return self._get_paginated_response(Account, path, page=page_number, pageSize=page_size,
